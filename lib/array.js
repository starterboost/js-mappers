...
ArrayMapper.prototype.update = function( items ){
	...
	_.each( items, (itemNew, index) => {
		...
		if( !itemOld ){
			//new item identified
			this.onAdd( itemNew, items );
		}else{
			//call update
			if( !this.isMatch( itemNew, itemOld ) ){
				this.onUpdate( itemNew, items );
			}
			//remove from our comparison list
			itemsOld = _.without( itemsOld, itemOld );
		}
	});

	//identify items that were not covere in the first loop
	_.each( itemsOld, ( itemOld ) => this.onRemove( itemOld, items ) );
